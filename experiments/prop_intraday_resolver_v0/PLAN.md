...
- **2d — pivot: resolver as CONDITIONER, not trade generator (NEXT; Ben's call 2026-06-14):** use the validated OFI break-classifier as a **filter on the existing tick-validated reclaim edge** (`mira_upgraded_v0/reclaim_entry.py`) — the resolver's original SPEC role (resolver → conditioner → governor). A reclaim long bets the level HOLDS; the resolver's `P(break)` / event-time OFI is exactly a "is the break real?" gate. Test: does OFI-class gating **lift the reclaim trades' OOS R by a day-block bootstrap CI clearing zero**? Reuse `reclaim_entry.py::seq_r` (honest tick fills) + the `combine_model.py` ablation harness. Prior is uncertain — memory says the MBO gate already HURT reclaim (feature ceiling), so a clean NULL is a live possibility; that's fine to document.
