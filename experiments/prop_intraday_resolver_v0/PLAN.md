...
- **2c — policy-geometry audit (DONE 2026-06-14 → NULL, adversarially verified):** `diag_policy.py` + the key-question diagnostic. **The naive OFI break-direction wrapper does NOT convert to a tradeable edge.** Honest 1R/1R race (`y_target_before_stop`) net-of-cost negative in every OFI bucket (low −0.215 / mid −0.100 / high −0.216); corr(OFI, realized_R) ≈ 0 → **OFI is a volatility proxy, not a directional R edge**; MFE ≈ MAE (no asymmetry); and the geometry/entry sweep is **un-runnable on the summary** (no tick ordering). A 3-lens verification workflow refuted the first optimistic "Path A" read (B/C/C, high conf) and caught a leaky lever ("drop the hold branch" uses post-hoc `y_hold`). See [`report/phase2c_policy.md`](report/phase2c_policy.md). **Phase-1 classifier (AUC 0.639) still stands** — it discriminates break-vs-hold but does not define a standalone trade.
- **2d — resolver as CONDITIONER (DONE 2026-06-14 → INCONCLUSIVE):** `conditioner_test.py` tested touch-OFI as an ex-ante gate on the reclaim edge. **No robust lift** (gate LOW −0.13 / HIGH −0.36 / baseline −0.24 OOS; corr +0.13 but sign-unstable vs the median split). The deeper finding: **the validated +0.5–0.8R reclaim edge is NOT present in this data slice** — `reclaim_entry.py` pooled OOS baseline is −0.01 [−0.24, +0.23], per-family all CIs straddle zero (ES-only / 2R / confirmation-universe / Apr–May; the headline edge was complex-wide / 3R / specific exits). So the conditioner can't be cleanly tested — there's no edge here to condition. Existing MBO gate doesn't rescue it either (feature ceiling). See [`report/phase2d_conditioner.md`](report/phase2d_conditioner.md). A clean test needs the complex-wide reclaim edge reproduced first (`mira_upgraded_v0` domain), then OFI + sweep-window-OFI gating — low prior.
- **deferred until a layer pays off:** the standalone multi-head LightGBM resolver and event-family expansion are on hold — there is no demonstrated standalone trade to model. Revisit only if the conditioner role pays.
