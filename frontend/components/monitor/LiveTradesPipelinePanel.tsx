...
function Body({ state }: { state: FetchState }) {
 ...
 <p className="text-base font-medium text-text">{summary}</p>
 </div>
 </div>
 <Stat
 label="Last import"
 ...
 <pre className="border border-border bg-surface p-2 tabular-nums text-[11px] text-text-dim whitespace-pre-wrap break-all max-h-48 overflow-y-auto">
 {data.import_log_tail.join("\n")}
 </pre>
 ) : null}

 <p className="tabular-nums text-[10px] text-text-mute break-all">
 <span className="text-text-mute">Inbox · </span>
 {data.inbox_dir}
 </p>
 </div>
 );
