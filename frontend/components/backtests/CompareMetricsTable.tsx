...
export default function CompareMetricsTable({
 ...
 <th className="px-3 py-2 text-left text-[10px] text-text-mute">
 Metric
 </th>
 <th className="px-3 py-2 text-right text-[10px] text-text-mute">
 {aLabel}
 </th>
 <th className="px-3 py-2 text-right text-[10px] text-text-mute">
 {bLabel}
 ...
 >
 <td className="px-3 py-1.5 text-text-dim">{row.label}</td>
...
function deltaToneFor(
 ...
 if (delta === null || better === "none" || delta === 0) return "text-text-dim";
 const improved =
 better === "higher" ? delta > 0 : delta < 0;
 return improved ? "text-pos" : "text-neg";
}
