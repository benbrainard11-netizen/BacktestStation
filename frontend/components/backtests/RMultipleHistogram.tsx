...
export default function RMultipleHistogram({ trades }: RMultipleHistogramProps) {
 ...
 <div className="flex flex-wrap items-center gap-x-4 gap-y-1 tabular-nums text-[11px] text-text-dim">
 <span>
 <span className="text-text-mute">wins</span>{" "}
 <span className="text-pos">{wins}</span>
 </span>
 <span>
 <span className="text-text-mute">losses</span>{" "}
 <span className="text-neg">{losses}</span>
 </span>
 {breakeven > 0 ? (
 <span>
 <span className="text-text-mute">breakeven</span>{" "}
 <span className="text-text-dim">{breakeven}</span>
 </span>
 ) : null}
 <span>
 <span className="text-text-mute">avg</span>{" "}
 <span className={avg > 0 ? "text-pos" : "text-neg"}>
 {avg > 0 ? "+" : ""}
 {avg.toFixed(2)}R
 </span>
 </span>
 ...
 <div className="flex justify-between border-t border-border px-3 py-1 tabular-nums text-[10px] text-text-mute">
