...
export default function RunBacktestForm({
 ...
 className="self-start tabular-nums text-[10px] text-text-mute hover:text-text-dim"
 >
 Switch to raw JSON →
 </button>
 </>
...
function SelectField({
 ...
 value={value}
 onChange={(e) => onChange(e.target.value)}
 className="border border-border bg-surface px-2 py-1.5 tabular-nums text-xs text-text focus:border-border focus:outline-none"
 >
 {options.map((opt) => (
...
function ErrorPanel({ message }: { message: string }) {
 ...
 <p className="mt-2 tabular-nums text-xs text-text">{message}</p>
 </div>
 );
}
