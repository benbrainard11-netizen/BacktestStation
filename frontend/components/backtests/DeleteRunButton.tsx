...
export default function DeleteRunButton({
 ...
 <span className="tabular-nums text-[11px] text-neg">{state.message}</span>
 <button
 type="button"
 onClick={cancel}
 className="self-start border border-border bg-surface px-2 py-0.5 tabular-nums text-[10px] text-text-dim"
 >
 close
 </button>
 </div>
 ...
 Type <span className="text-neg">{confirmPhrase}</span> to confirm
 </span>
 ...
 disabled={disabled}
 className="w-56 border border-border-strong bg-surface px-2 py-1 tabular-nums text-xs text-text placeholder:text-text-mute focus:border-neg/30 focus:outline-none disabled:opacity-60"
 ...
 className={cn(
 "border px-2 py-1 tabular-nums text-[10px] ",
 matches && !disabled
 ? "border-neg/30 bg-neg/10 text-neg hover:bg-neg/10"
