  ...
  return (
    <section className="grid grid-cols-1 gap-6 lg:grid-cols-12 lg:gap-12">
      <div className="hidden items-center lg:col-span-1 lg:flex">
        <span aria-hidden="true" className="block h-px w-full bg-zinc-100/40" />
      </div>
      <blockquote
        className={cn(
          "lg:col-span-9",
          "font-extralight italic leading-[1.15] tracking-tight text-zinc-200",
        )}
        style={{ fontSize: "clamp(1.6rem, 3vw, 2.6rem)" }}
      >
        <span className="text-zinc-700">“</span>The modal failure at this risk
        level is{" "}
        <span className="text-rose-300">{phrase}</span>
        <span className="text-zinc-500"> — </span>
        <span className="tabular-nums text-zinc-100">{ratePct}%</span> of
        sequences die there.<span className="text-zinc-700">”</span>
      </blockquote>
      <div className="hidden flex-col items-end justify-end lg:col-span-2 lg:flex">
        ...
        <span className="text-[10px] uppercase tracking-[0.32em] text-zinc-700">
          0001
        </span>
      </div>
    </section>
