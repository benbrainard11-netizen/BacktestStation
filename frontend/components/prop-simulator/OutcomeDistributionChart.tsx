...
export default function OutcomeDistributionChart({
 ...
 {hovered ? (
 <div
 className="pointer-events-none absolute -top-1 -translate-x-1/2 -translate-y-full whitespace-nowrap rounded-md border border-border-strong bg-surface px-2.5 py-1.5 tabular-nums text-[10px] text-text"
 ...
 <div className="flex flex-wrap gap-x-4 gap-y-1 tabular-nums text-[10px] text-text-dim">
 <span className="flex items-center gap-1.5">
 <span aria-hidden="true" className="h-[3px] w-4 rounded-sm bg-pos" />
 Mean
 </span>
 <span className="flex items-center gap-1.5">
 <span
 ...
 <span
 aria-hidden="true"
 className="h-2.5 w-3 rounded-sm bg-border-strong ring-1 ring-zinc-600/50"
 />
 ±1σ band
 </span>
 <span className="flex items-center gap-1.5">
 <span aria-hidden="true" className="h-[3px] w-4 rounded-sm bg-text-mute" />
 P10 / P90 ticks
 </span>
 <span className="ml-auto text-text-mute">hover bars for bucket counts</span>
 </div>
 </div>
 );
}
