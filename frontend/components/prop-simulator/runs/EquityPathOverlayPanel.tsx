...
const BUCKET_TONE: Record<SelectedPath["bucket"], string> = {
  ...
  near_pass: "stroke-emerald-500/60",
};
          ...
          {paths.map((path) => (
            <span key={path.bucket} className="flex items-center gap-1.5">
              <span
                ...
                className={cn(
                  "h-[3px] w-4",
                  path.bucket === "best" && "bg-emerald-400",
                  path.bucket === "near_pass" && "bg-emerald-500/60",
                  path.bucket === "median" && "bg-zinc-300",
                  path.bucket === "near_fail" && "bg-rose-500/60",
                  path.bucket === "worst" && "bg-rose-400",
                )}
              />
