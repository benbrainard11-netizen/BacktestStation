...
const BUCKET_TEXT: Record<SelectedPath["bucket"], string> = {
 best: "text-pos",
 near_pass: "text-pos/85",
 median: "text-text",
 near_fail: "text-neg/85",
 worst: "text-neg",
};
...
export default function EquityOverlayChart({
 ...
 if (paths.length === 0) {
 return (
 <div className="flex h-40 items-center justify-center tabular-nums text-xs text-text-mute">
 No paths selected.
 </div>
 );
 }
 ...
 doesn't jump. */}
 <div className="flex h-5 flex-wrap items-center gap-x-3 gap-y-1 tabular-nums text-[10px] text-text-mute">
 {hoverDay !== null ? (
 <>
 <span className="text-text-dim">day {hoverDay}</span>
 ...
 <span className="text-text-mute">hover chart for crosshair</span>
 )}
 </div>
