...
4. **Schemas have one source.** Pydantic models in `backend/app/schemas/` are the truth. After the 2026-06-08 backend-only refactor the web API + its TS type-gen pipeline were removed, so the only remaining schema is `research_events` (emitted by the research detectors). No `generate-types`, no `shared/openapi.json`, no frontend types.
...
Auth, billing, Docker, Kubernetes, Postgres, walk-forward, websockets, mobile UI, broker execution integration, no-code strategy builders, and a web UI / CRUD API (removed 2026-06-08 — this repo is backend + tools + a read-only status page).
...
- Pre-commit runs ruff + black (Python). Don't bypass with `--no-verify`.
...
The only HTTP surface is the read-only status page (`backend/app/main.py`) — there is no API client to regenerate.
...
1. **Backend + tools only.** There is no web app (removed 2026-06-08). Don't add CRUD/HTTP endpoints or a UI; new capability is a tool (a CLI or importable Python module), and research goes in `experiments/` or `market_state/`.
2. **The status page stays read-only.** `backend/app/main.py` serves only the status dashboard. Don't bolt tool-execution endpoints onto it — run tools from the command line.
...
5. **Tests before commit.** `pytest -q` green (backend). Tools run via CLI/Python; the only UI is the read-only status page at `/`.
6. **Pipeline silent failures = active problem.** When a daily-fire scheduled task or live ingester logs `errors=0` but produces no output, that's a bug, not "expected idle." Surface it on the status page.
