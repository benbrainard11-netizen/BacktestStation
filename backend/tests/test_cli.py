...
def test_data_validate_invokes_runner_script(monkeypatch) -> None:
    """`bs data validate` should shell out to validate_snapshot.py.

    We monkeypatch subprocess.run to assert the right script + args are
    invoked, returning a synthetic non-zero exit (e.g. snapshot missing)
    so we exercise the error-propagation path too.
    """
    from scripts.cli import cmd_data
    ...
    def fake_run(args, **kwargs):
        ...
        return subprocess.CompletedProcess(
            args=args, returncode=1, stdout="", stderr="ERROR: snapshot 'snapshot-1' not found\n"
        )

    monkeypatch.setattr(cmd_data.subprocess, "run", fake_run)
    result = runner.invoke(app, ["data", "validate", "snapshot-1"])
    assert result.exit_code == 1
    assert "validate_snapshot.py" in " ".join(captured["args"])
    assert "snapshot-1" in captured["args"]
