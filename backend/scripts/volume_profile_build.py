"""Run volume_profile scans + outcomes for all 5 modes."""
...
SCAN_TASKS = [
    ("vp daily",   "volume_profile", "daily_volume_profile"),
    ("vp weekly",  "volume_profile", "weekly_volume_profile"),
    ("vp asia",    "volume_profile", "asia_volume_profile"),
    ("vp london",  "volume_profile", "london_volume_profile"),
...
def setup_logging() -> Path:
    ...
    log_path = LOG_DIR / f"volume_profile_{ts}.log"
    logging.basicConfig(
        level=logging.INFO,
...
def run_outcomes():
    cmd = [
        ...
        "--computer", "volume_profile_reactions_v2",
    ]
    logging.info("START outcomes: volume_profile_reactions")
    t0 = time.time()
    proc = subprocess.run(cmd, capture_output=True, text=True, timeout=14400)
    elapsed = time.time() - t0
    if proc.returncode != 0:
    ...
    try:
        ...
        logging.info(
            ...
            f"n_updated={summary.get('n_updated')} ({elapsed:.0f}s)"
        )
    except Exception:
...
def main():
    ...
    logging.info(f"volume_profile build started. Log: {log_path}")
    ...
    logging.info("volume_profile build done")
    return 0
